...
from model_helpers import SklearnHelper
...
def gridsearch_cv(x, y, x_shape):
    ...
    keras_model1 = KerasClassifier(build_fn=create_keras1)
    keras_model2 = KerasClassifier(build_fn=create_keras2)

    k1 = GridSearchCV(estimator=keras_model1, verbose=0, param_grid=_PARAMS_KERAS, cv=strat_kfold, scoring='accuracy',
                      n_jobs=1)

    k1_results = k1.fit(X=x, y=y)
    k1_params = k1_results.best_params_
    print('Best k1 params: %s' % k1_results.best_params_)

    k2 = GridSearchCV(estimator=keras_model2, verbose=0, param_grid=_PARAMS_KERAS2, cv=strat_kfold, scoring='accuracy',
                      n_jobs=1)

    k2_results = k2.fit(X=x, y=y)
    k2_params = k2_results.best_params_
    print('Best k2 params: %s' % k2_results.best_params_)

    xg1 = GridSearchCV(estimator=xgb.XGBClassifier(), param_grid=_PARAMS_XGBOOST, cv=strat_kfold, scoring='accuracy',
                       n_jobs=1)
    xg1_results = xg1.fit(X=x, y=y)
    xg1_params = xg1_results.best_params_
    print('Best xg1 params: %s' % xg1_results.best_params_)
...
def fit_models(k1_params, k2_params, xg1_params, rfc1_params, etc1_params, abc1_params, gbc1_params, svc1_params, x, y,
               x_pred, df_predict):
    k1 = KerasClassifier(build_fn=create_keras1, **k1_params)
    k2 = KerasClassifier(build_fn=create_keras2, **k2_params)
    xg1 = xgb.XGBClassifier(random_state=_STRAT_KFOLD_PARAMS.get('random_state'), **xg1_params)
...
def _stacked_xgboost(x_train, y_train, x_pred, df_predict):
    gbm = xgb.XGBClassifier(**_PARAMS_XGBOOST_SECOND_LEVEL).fit(x_train, y_train)
